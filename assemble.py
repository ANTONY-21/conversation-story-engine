...
VIDEO = f'{PROJ}/out/convo_scenes.mp4'
MUSIC = '/opt/kinocut-work/ch02_video1/audio/music_bed_v4.wav'
FINAL_V = f'{PROJ}/out/chai_inflation_master.mp4'
FINAL_H = f'{PROJ}/out/chai_inflation_master_horizontal.mp4'
...
for i, p in enumerate(plan):
    ...
    spk = SPEAKERS[p['id']]
    if spk == 'VIKRAM':
        # pitch down 6% for character separation; atempo restores speed
        # VO wavs are 22050 Hz mono (verified via ffprobe)
        chain = f"asetrate=22050*0.94,aresample=22050,atempo={1/0.94:.5f},"
    else:
        chain = ''
    out_label = f'vo{i}'
    filters.append(
    ...
    prev = f'a{i}'
filters.append(f"[9:a]volume=0.12,apad[m];[{prev}][m]amix=inputs=2:duration=first:normalize=0[final]")
